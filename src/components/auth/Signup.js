...
import { register } from '../../state/actions/layout'

const Signup = ({ setLogin }) => {
  const [userData, setUserData] = useState({ name: 'Kaven Appo', email: 'kavenappo@gmail.com', password: 'w@Xb2GyakdVYt@M', cPassword: 'w@Xb2GyakdVYt@M', error: null, success: null })

  const dispatch = useDispatch()

  const authInputHandle = (e) => {
    const toSend = { ...userData, [e.target.name]: e.target.value, success: null, error: {} }
    ...
    <Fragment>
      ...
      <form autoComplete="off" className="space-y-4">
        ...
        <div className="flex flex-col">
          ...
          <input onChange={authInputHandle} value={userData.cPassword} type="password" id="cPassword" className={`${userData.error?.cPassword ? "border-red-500" : ""} px-4 py-2 focus:outline-none border`} />
